...
module.exports = class ResponseInterface 
{
   ...
   * @deprecated We only changed the function name. Its functionality and usage remains the same
   * */
  header (key, value)
  {
    console.warn("header method is deprecated. Consider using setHeader(key, value) instead")
    this.headers[key] = value;
  }

  getHeader = (key) => this.headers.get(key)
  ...
  /**
   * 
   * @param {any} data - Data that would be sent to client.
   * @description - That is last step before sending data to the client. Consider doing all manipulations before that step 
   */
  complete(data) {
